...
public class Launcher : MonoBehaviourPunCallbacks
{
    ...
    void Awake()
    {
        ...
        {
            GameObject entry = Instantiate(levelPrefab);
            entry.transform.SetParent(levelListPanel.transform);
            entry.GetComponent<LevelEntry>().Initialize(levelNames[i], this);
        }
        selectedScene = levelNames[0];
    }
    ...
    {
        SetActivePanel(levelPanel.name);
    }

    public void ButtonCreateRoom(string levelName)
    {
        SetActivePanel(connectLabel.name);
